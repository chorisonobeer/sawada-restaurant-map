...
const Content = (props: Props) => {
  ...
  const isSafeMode = props.safeMode === true;

  // 表示項目
  const hoursRawVal = props.data['営業時間'];
  const hoursRaw = typeof hoursRawVal === 'string' ? hoursRawVal : '営業時間不明';
  const hours = removeWeekdaysFromHours(String(hoursRaw));
  const hourRanges = hours.split(/\s*,\s*/).filter(Boolean);
  const closedVal = props.data['定休日'];
  const closed = typeof closedVal === 'string' ? closedVal : '定休日不明';
  const introVal = props.data['紹介文'];
  const intro = typeof introVal === 'string' ? introVal : '';

  return (
    <div className="shop-link">
      {typeof props.data['エリア'] === 'string' && props.data['エリア'] && (
        <div className="area-badge">{props.data['エリア']}</div>
      )}
      <h2 className="shop-title" style={{ wordBreak: "break-all" }} onClick={clickHandler}>
        {String(props.data['スポット名'] || '')}
      </h2>
