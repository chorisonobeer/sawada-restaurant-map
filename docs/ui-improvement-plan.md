...
## 実装プラン再評価（崩れ対策の修正方針）
**現状の課題**: v2.0 プランのまま大きくスタイルを差し替えると、依存関係（高さ・z-index・位置指定）が一気に崩れ、Map/検索UI/フッターの整合が取れなくなる恐れが高いことが判明しました。

**厳しめの自己採点（10点満点）**
- リスク評価の明示: 6/10（崩れリスクの具体例が不足）
- 依存関係の保持策: 5/10（高さ/余白/z-indexの連鎖に対する手当てが弱い）
- スコープ管理: 6/10（Phase1での変更範囲が広すぎる）
- フォールバック/ロールバック: 5/10（戻し手順が曖昧）
- 実装順序の安全性: 6/10（Map高さ計算やフッター依存を守る指針が不足）

**崩れを防ぐための修正ポイント（実装前のプラン修正）**
1. **高さ・位置計算の原則を固定**  
   - `app-body` とフッターの差分計算（60px + safe-area）は維持する。  
   - Mapの高さ指定（`calc(100% - 50px/60px)` 系）は、Phase1では変更しない。  
   - `.shop-single` の `bottom: 60px` など、既存依存関係を崩さずに着手する。
2. **スコープを絞った段階実装に再分割（Phase1.1→1.2→1.3）**  
   - Phase1.1: デザイントークンの「色・余白・角丸・シャドウ・z-index」だけ導入（サイズや位置の計算式は触らない）。  
   - Phase1.2: ボトムナビの色/角丸/フォントをトークン適用（高さやグリッドは触らない）。  
   - Phase1.3: 検索バーのスタイルのみ軽微に（色/角丸/シャドウ）。位置・幅の計算式は現状維持。  
   - チップ化や位置大変更、グラスモーフィズム大量適用は **Phase2 以降** に後ろ倒し。
3. **clamp/計算式の適用を最小限にし、段階導入**  
   - まずは色・角丸・シャドウのみトークン化。`calc()` と `clamp()` の適用は小さな要素（文字サイズや余白）から試す。  
   - 幅・位置の `calc(...)` をいきなり置き換えない（崩れの主因になりやすい）。
4. **z-indexの整理を「付け替え」だけに限定**  
   - 新規トークンの `map/search/footer/modal` を既存値に対応づけるだけにし、値自体を大きく上下させない。  
   - 位置変更や新しい重なりを伴う変更は後続フェーズへ。
5. **フォールバックとロールバック手順を明記**  
   - 各サブフェーズごとに「1コミット→表示確認→OKなら次へ」。  
   - 崩れた場合は直前コミットへ即戻せるように、変更単位を細かく保つ。

**修正後の安全な実装順序（改訂版）**
- Phase1.1: `design-tokens.scss` を色/角丸/シャドウ/z-index/spacing のみに限定して導入。既存スタイルをトークン参照に差し替えるが、位置や高さの計算式には触れない。
- Phase1.2: `Tabbar.scss` の色・フォント・角丸・シャドウのみトークン適用。高さやグリッド構造は現状維持。
- Phase1.3: `SearchFeature.scss` の色・角丸・シャドウのみトークン適用。`top/left/width` の計算式・配置は触れない。
- Phase2以降: チップ化、幅/位置の `calc` 再設計、グラスモーフィズム強化、検索バー位置大変更などを小刻みに実施。

**着手前のチェックリスト（再掲・強化）**
- Map高さ計算とフッター依存（60px + safe area）を崩さない。
- `.shop-single` の bottom と `.app-footer` の高さ連動を維持。
- z-index 値の再定義は「意味マッピング」のみに留め、数値の大幅変更はしない。
- clamp 導入は文字サイズ・余白などから試し、レイアウト位置は後続フェーズ。
- 1変更1コミットでロールバック容易性を確保。
